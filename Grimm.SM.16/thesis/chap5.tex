...
Instead of using a single update function as in \Cref{eqn8}, I model the problem as solving two separate update functions over the same state-space:
...
Tensor-based value iteration can be modified to solve pursuit-evasion games using a best response dynamic. As in \cite{Gorod}, tensor-train cross can be used to provide an approximation of value iteration within an error bound $\epsilon$. By applying tensor-train cross to \Cref{DPBRalg}, a best response TVI algorithm results as in \Cref{BRTVIalg}.
\begin{algorithm}
