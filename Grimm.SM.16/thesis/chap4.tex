...
It is further shown in \cite{Gorod} that the error of tensor-based value iteration can be bounded.
\begin{theorem}[TVI Approximation Error \cite{Gorod}]
...
The key to tensor-train value iterations success is the reduction in the number of states examined. Of the $3.4 \times 10^10$ states making up the initial discretized state space, only approximately $10^6$ of these states are used as can be seen in \Cref{gorodstates}.\cite{Gorod}
\begin{figure}
\centering
